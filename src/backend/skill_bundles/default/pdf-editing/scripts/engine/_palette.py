...
def _normalize_hex(h: str) -> str:
    """Return a canonical 6-digit lowercase hex string (no leading ``#``).

    Accepts both the 3-digit CSS shorthand (``#0a5``) and the full 6-digit form
    (``#00aa55``). The shorthand expands each digit (``r`` → ``rr``) per the
    CSS Color spec — so ``#0a5`` and ``#00aa55`` mean the same colour. Raises
    ValueError with a clear message on any other shape rather than dying with
    ``int('', 16)`` deep inside the cover renderer.
    """
    if not isinstance(h, str) or not h:
        raise ValueError(f"hex colour must be a non-empty string, got {h!r}")
    raw = h.lstrip("#")
    if len(raw) == 3 and all(c in "0123456789abcdefABCDEF" for c in raw):
        return (raw[0] * 2 + raw[1] * 2 + raw[2] * 2).lower()
    if len(raw) == 6 and all(c in "0123456789abcdefABCDEF" for c in raw):
        return raw.lower()
    raise ValueError(
        f"hex colour must be 3- or 6-digit (e.g. '#0a5' or '#00aa55'), got {h!r}"
    )
...
def build_tokens(
    ...
    # Apply caller-supplied overrides before token assembly. Normalize the
    # hex string here so downstream consumers (cover HTML, body PDF) all see
    # the canonical 6-digit form regardless of whether the caller passed
    # ``#0a5`` or ``#00aa55``.
    if accent_override:
        canonical_accent = "#" + _normalize_hex(accent_override)
        palette["accent"]    = canonical_accent
        palette["accent_lt"] = _lighten(canonical_accent, 0.09)
    if cover_bg_override:
        palette["cover_bg"] = "#" + _normalize_hex(cover_bg_override)
