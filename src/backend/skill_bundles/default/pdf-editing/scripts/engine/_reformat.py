...
Supported input formats:
  ...
  .docx         — Word document (headings, paragraphs, lists, tables preserved)
...
def ensure_deps():
    ...
    if missing:
        raise ImportError(
            f"the engine.pdf reformat requires: {', '.join(missing)} "
            "(add to requirements-mcp.txt)"
        )
...
def parse_docx(docx_path: str) -> list:
    """
    Parse a Word .docx file into content.json blocks.
    Preserves headings (h1/h2/h3), paragraphs, bullet/numbered lists, and tables.
    """
    ...
    for element in doc.element.body:
        ...
        if tag == "p":
            ...
            if style_name.startswith("heading"):
                # "Heading 1" → h1, "Heading 2" → h2, etc.
                try:
                    level = int(style_name.split()[-1])
                except (ValueError, IndexError):
                    level = 1
                level = min(level, 3)
                blocks.append({"type": f"h{level}", "text": _docx_rich_text(para)})

            elif style_name.startswith("title"):
                blocks.append({"type": "h1", "text": _docx_rich_text(para)})

            elif "list bullet" in style_name or style_name.startswith("list bullet"):
                blocks.append({"type": "bullet", "text": _docx_rich_text(para)})

            elif "list number" in style_name or style_name.startswith("list number"):
                blocks.append({"type": "numbered", "text": _docx_rich_text(para)})

            elif style_name in ("quote", "intense quote", "block text"):
                blocks.append({"type": "callout", "text": _docx_rich_text(para)})

            else:
                # Check outline level from XML for headings without named styles
                pPr = element.find(".//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}pPr")
                outlineLvl = None
                if pPr is not None:
                    olvl = pPr.find("{http://schemas.openxmlformats.org/wordprocessingml/2006/main}outlineLvl")
                    if olvl is not None:
                        outlineLvl = int(olvl.get("{http://schemas.openxmlformats.org/wordprocessingml/2006/main}val", "9"))

                # Check for bullet/numbering via numPr
                numPr = None
                if pPr is not None:
                    numPr = pPr.find("{http://schemas.openxmlformats.org/wordprocessingml/2006/main}numPr")

                if outlineLvl is not None and outlineLvl < 3:
                    blocks.append({"type": f"h{outlineLvl + 1}", "text": _docx_rich_text(para)})
                elif numPr is not None:
                    blocks.append({"type": "bullet", "text": _docx_rich_text(para)})
                else:
                    blocks.append({"type": "body", "text": _docx_rich_text(para)})
...
def _docx_rich_text(para) -> str:
    """Extract paragraph text with bold/italic preserved as ReportLab XML markup."""
    parts = []
...
def parse_file(input_path: str) -> tuple[list, list]:
    ...
    return [], [f"Unsupported file type: {ext}. Supported: .md .txt .docx .pdf .json"]
