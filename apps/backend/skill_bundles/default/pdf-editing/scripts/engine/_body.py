...
def ensure_deps():
    # In the MCP container these are guaranteed by requirements-mcp.txt.
    # Never shell out to pip from a long-lived server process.
    missing = [p for p in ("reportlab", "pypdf")
               if importlib.util.find_spec(p) is None]
    if missing:
        raise ImportError(
            f"the engine.pdf body renderer requires: {', '.join(missing)} "
...
def _content_has_cjk(content: list) -> bool:
    for item in content:
        for key in ("text", "title"):
            if isinstance(item.get(key), str) and _has_cjk(item[key]):
                return True
        for h in item.get("headers", []):
            if isinstance(h, str) and _has_cjk(h):
                return True
        for row in item.get("rows", []):
            for cell in row:
                if isinstance(cell, str) and _has_cjk(cell):
                    return True
    return False
...
def register_fonts(tokens: dict, content: list | None = None):
    ...
    When CJK characters are detected in *content*, automatically register
    ...
    if content and _content_has_cjk(content):
        for candidate in _cjk_font_candidates():
            if not os.path.exists(candidate):
                continue
            try:
                is_ttc = candidate.lower().endswith(".ttc")
                if is_ttc:
                    pdfmetrics.registerFont(TTFont("CJK-Regular", candidate, subfontIndex=0))
                else:
                ...
                try:
                    if is_ttc:
                        pdfmetrics.registerFont(TTFont("CJK-Bold", candidate, subfontIndex=0))
                    else:
                        pdfmetrics.registerFont(TTFont("CJK-Bold", candidate))
                except Exception:
                    pdfmetrics.registerFont(TTFont("CJK-Bold", candidate, subfontIndex=0) if is_ttc
                                            else TTFont("CJK-Bold", candidate))

                # Override token font names so all styles use the CJK font
                tokens["font_body_rl"] = "CJK-Regular"
                tokens["font_body_b_rl"] = "CJK-Bold"
                tokens["font_display_rl"] = "CJK-Bold"
                print(f"[render_body] CJK font registered: {candidate}", file=sys.stderr)
                break
...
class BibliographyItem(Flowable):
    ...
    def draw(self):
        ...
        label_font = _CJK_FONT_NAME or "Helvetica-Bold"
...
def make_styles(t: dict) -> dict:
    ...
    return {
        ...
        "code": ParagraphStyle("Code",
            fontName=_CJK_FONT_NAME or "Courier", fontSize=8.5, leading=12.5,
            textColor=HexColor(dk),
        ),
        "code_lang": ParagraphStyle("CodeLang",
            fontName=_CJK_FONT_NAME or "Courier", fontSize=7, leading=10,
        ...
        "math_fallback": ParagraphStyle("MathFb",
            fontName=_CJK_FONT_NAME or "Courier", fontSize=9, leading=13,
            textColor=HexColor(dk),
        ),
        "eq_label": ParagraphStyle("EqLabel",
            fontName=_CJK_FONT_NAME or "Helvetica", fontSize=9, leading=12,
            textColor=HexColor(mu),
        ),
    }
