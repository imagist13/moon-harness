...
def find_ws_path(work_dir: str, sheet_name: str | None) -> str:
    ...
    for rel in rels_tree.getroot():
        if rel.get("Id") == rid:
            ...
            # or relative to this .rels file's directory ("worksheets/sheet1.xml").
...
def main() -> None:
    ...
    # Step 5: Update dimension to cover the inserted row.
    #
    # shift_rows only grows <dimension> when it actually shifts existing rows
    # down — i.e. for an insert *inside* the used range. Appending a row at (or
    # past) the last row shifts nothing, so the old ref stays too small. Excel
    # re-derives the used range on open and shows the row regardless, but
    # SheetJS-based viewers (the in-app preview) trust <dimension> verbatim and
    # silently drop any row outside it. So extend it to include row `at`.
    for dim in root.iter(_tag("dimension")):
        ...
        start_row = min(start_row, at)
        end_row = max(end_row, at)
        if all_cols and col_number(all_cols[-1]) > col_number(end_col):
            end_col = all_cols[-1]  # all_cols is sorted by column number
