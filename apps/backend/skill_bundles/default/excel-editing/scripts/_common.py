"""Shared utilities for the excel-editing skill scripts.
    ...
    *,
    ...
    Args:
        inputs: ``{basename_in_workdir: absolute_source_path}``. Each source
            ...
            Pass ``None`` or empty dict for create-from-scratch flows.
    ...
    with tempfile.TemporaryDirectory(prefix="excel_skill_") as wd_str:
        workdir = Path(wd_str)
...
def load_json_arg_or_file(
    ...
    Exactly one must be provided. Used for large payloads (sheets / patches /
    model spec) that may exceed the bash ``Argument list too long`` cap.
    """
...
def _refuse_direct_run() -> None:
    ...
    *is* it. Without this guard, ``python scripts/_common.py excel-cli
    ...
    # ``python _common.py excel-cli create …`` → suggest ``excel-cli create …``.
    while argv and argv[0] in ("excel-cli", "cli.py", "cli", "_common.py"):
        argv = argv[1:]
    suggested = "excel-cli " + " ".join(argv) if argv else "excel-cli <subcommand> [args]"
    emit_error(
        "NotAnEntryPoint",
        "_common.py is a shared library for the excel-editing skill, not a "
        "runnable command — nothing was executed and no file was produced. "
        "`excel-cli` is already on PATH; never run `python scripts/*.py` "
