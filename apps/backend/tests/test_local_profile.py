...
def test_ce_branding_repairs_persistent_page_config(db_session, monkeypatch):
    ...
    assert row.payload["branding"] == {
        "product_name": "Lumen OS",
        "product_subtitle": "AI 智能助手",
        "page_title": "Lumen OS",
        "hero_title": "你好，我是 Lumen OS",
        "logo_url": "/custom.svg",
    }
    assert row.payload["navigation"]["admin_header"]["title"] == "Lumen OS — 后台管理"
    assert row.payload["navigation"]["admin_platform"]["product_name"] == "Lumen OS"
...
async def test_ce_local_login_starts_in_english_with_brand_title(monkeypatch):
    ...
    monkeypatch.setattr(
        mock_sso,
        "get_branding_info",
        lambda: {"product_name": "Lumen OS", "logo_url": "/icon.png"},
    )
    request = Request(
        {
            "type": "http",
    ...
    assert "<title>Lumen OS</title>" in html
    assert 'data-tab="register"' not in html
    assert 'name="confirm_password"' not in html
    assert '<img class="brand-logo" src="/home/hugagentos-logo.png" alt="Lumen OS" />' in html
    assert "<span>Lumen OS</span>" not in html
    assert "document.title = 'Lumen OS'" in html
...
def test_runner_canon_ws_and_bash_rewrite(monkeypatch):
    ...
    local_root = "/Users/test/Library/Application Support/Lumen OS/workspace"
