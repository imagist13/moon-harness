...
通过沙箱内的 `dws`（DingTalk Workspace CLI）命令管理钉钉产品能力。`dws` 二进制已随沙箱镜像预装，无需安装。

## 连接前置（必须先确认）
本技能以**当前用户的钉钉身份**执行命令，前提是该用户已在「设置 → 集成 → 连接钉钉账号」完成授权（凭据已注入沙箱）。
- 执行任意 `dws` 业务命令前，先用 `dws auth status --format json` 确认已登录；若返回未登录/无 refresh token，**停止并提示用户先去设置面板连接钉钉账号**，不要尝试在聊天里发起登录。
- 若某命令以退出码 `2`（Auth）失败，等同未连接，按上一条处理。
- 若某命令以退出码 `4`（PAT 授权拦截）失败，stderr 会带一行 `PAT_AUTHORIZATION_URL=<url>`：把该链接原样给用户，请其在钉钉中点击同意授权后，再重试原命令。**不要**绕过授权或改用其它方式。

> 凭据由后端通过 per-user 持久卷挂载到沙箱（`~/.dws` + `~/.local/share/dws-cli`），跨会话存活；token 刷新由 `dws` 自动完成并回写。技能本身不读写、不导出凭据明文。


> ⚠️ **命令可用性可能因企业服务发现配置而异**。本文档列出的命令基于 dws envelope schema 实测，但部分命令的子命令暴露与否还取决于你的企业 MCP gateway 是否注册了对应 tool。如果跑某条命令报 `unknown command` 或 fall back 到父级 help，说明当前账号企业未开通该能力。实际调用前可用 `dws <cmd> --help` 或 `--dry-run` 验证。
