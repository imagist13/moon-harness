"""Lumen OS no-Docker local CLI (``hugagent`` console entry).
...
def cmd_serve(args) -> int:
    ...
    print(f"Lumen OS 监听于 http://{host}:{port}/  (Ctrl-C 停止)")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="hugagent", parents=[common], description="Lumen OS 无 Docker 本地版"
    )
